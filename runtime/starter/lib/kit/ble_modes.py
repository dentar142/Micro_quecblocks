...
class EasyBLE:
    ...
    def send(self, data, handle=None):
        ...
        if self.mode == "client":
            handle = getattr(self.config, "BLE_CLIENT_VALUE_HANDLE", None) if handle is None else handle
            return False if handle is None else self.write_handle(handle, text)
    ...
    def status(self):
        ...
        if self.mode == "client":
            data.update({"target_name": self.target_name, "connected": self.connected, "conn_id": self.conn_id, "mtu": self.mtu, "target_addr": self.target_addr, "scan_count": len(self.scan_results), "services": self.services, "chars": self.chars, "last_value": self.last_value, "last_event": self.last_event})
        else:
            data.update({"addr": self.addr, "connected": self.connected, "notify_enabled": self.notify_enabled, "last_value": self.last_value, "last_event": self.last_event})
        return data
