...
def _stop_led_pwm_if_needed(name):
    if str(name).lower() == "green":
        _stop_led_pwm()


def ledbrightness(name="green", duty=100, freq=None):
    """Set LED brightness; green/LED1 uses hardware PWM on PB0.

    Blue and red are digital-only on the supplied kit. For those channels,
    duty below 50 turns the LED off and duty at/above 50 turns it on.
    """
    ...
    if channel != "green":
        _stop_led_pwm()
        _led_breathe_state = None
        return setled(channel, 1 if duty >= 50 else 0)
    try:
        _led_breathe_state = None
        if duty <= 0:
            _stop_led_pwm()
        ...
        if _led_pwm is None:
            ...
            _led_pwm = PwmOutput(config.LED1_PWM_PIN, freq or config.LED_PWM_FREQ, duty)
            _led_pwm.start()
        else:
            _led_pwm.set_duty(duty)
        return True
    except Exception as exc:
        _stop_led_pwm()
        _remember_error("leds", exc)
        return _fail("LED1_PWM", exc)


def ledbreathe(name="green", period_ms=None, min_duty=0, max_duty=100, steps=32):
    """Start a non-blocking breathe cycle; call updateled() in a fast loop."""
    global _led_breathe_state
    channel = str(name or "green").lower()
    if channel != "green":
        return _skip("LED_BREATHE", "only LED1/green PB0 has hardware PWM")
    ...
    current = _led_breathe_state
    if current and current.get("name") == channel and current.get("period") == period and current.get("low") == low and current.get("high") == high and current.get("steps") == count:
        # Repeated calls from a user loop must not restart the animation.
        return updateled()
    # Set the initial duty without clearing the state we just installed.
    _led_breathe_state = {"name": channel, "period": period, "low": low, "high": high,
...
def updateled():
    """Advance a running non-blocking LED1 breathe animation."""
    global _led_breathe_state
    state = _led_breathe_state
    if not state:
        return False
    now = ticks_ms()
    ...
    # ledbrightness() intentionally cancels an active animation when called
    # by user code; preserve the state while the animation updates its duty.
    saved_state = state
    ledbrightness(state["name"], state["low"] + span * phase / state["steps"])
    _led_breathe_state = saved_state
    return True
