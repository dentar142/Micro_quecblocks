...
def uart(enabled=1, uart_id=None, baudrate=None, timeout=None):
    global _uart, _uart_id, _uart_baudrate, _uart_timeout, _uart_text_buffer
    changed = False
...
def senduart(data):
    ...
    if not port:
        return None
    payload = data
    if getattr(config, "UART_AUTO_LINE_COMMANDS", True) and isinstance(data, str):
        if data in ("ON", "OFF", "OK"):
            payload = data + "\r\n"
    port.write(_payload(payload))
    _mirror_uart_to_pc(payload)
    return True
...
def readuarttext():
    ...
    if isinstance(data, (bytes, bytearray)):
        ...
        text = str(data)
    # UART command tools normally append CR/LF, but a USB bridge may split a
    # line across reads. Keep a small text buffer until a terminator arrives.
    had_pending = bool(_uart_text_buffer)
    _uart_text_buffer += text
    for index, char in enumerate(_uart_text_buffer):
        if char in "\r\n":
            line = _uart_text_buffer[:index]
            remainder = _uart_text_buffer[index + 1:]
            if remainder.startswith("\n") or remainder.startswith("\r"):
                remainder = remainder[1:]
            _uart_text_buffer = remainder
            return line
    if had_pending:
        return None
    if _uart_text_buffer.endswith(("LED_", "LED", "CMD_")):
        return None
    # Preserve the original non-line message behavior for sensors/tools that
    # intentionally send a short payload without a terminator.
    _uart_text_buffer = ""
    return text
